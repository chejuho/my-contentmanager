<tiles:insert template="/WEB-INF/view/common/table_layout.jsp" flush="true">
	<tiles:put name="title" value="データ編集画面" />
	<tiles:put name="javascripctInput" type="string">
		...
		<script src="${f:url('/js/jHtmlArea.ColorPickerMenu-0.7.0.js')}" type="text/javascript" ></script>
		...
		jHtmlAreaColorPickerMenu.defaultOptions.colors = ${colors};
		  ...
		  $(document).ready(function(){
		    ...
		    $("div.emoji_palette td").bind("click", function(event){
				...
				//var iframe = $('#iframeid' + field);
				//var doc = iframe[0].contentWindow.document;
				//doc.body.innerHTML = doc.body.innerHTML + value;
	...
	<tr>
		<td>
			<table>
				<td>
					<s:link href="detail/${tablename}/${pageNum}"> データリスト&nbsp;</s:link>
				</td>
	...
	<tr>
		<td>
			<table id="inputtable" width="80%" border="1" cellspacing="0" cellpadding="0">
			<tr>
			...
			<tr>
				<th width="30%"  class="table-title-text">
				...
				<td width="70%" bgcolor="#FFFFFF" class="table-cont-text">
					<!--  主キーの場合非活性化 -->
					<c:if test="${column.primary_key_sign == 1}">
           				<input type="text" name="${f:h(field)}" value="${f:h(dataMap[field])}"  class="updateinput" disabled />
           				<input type="hidden" name="${f:h(field)}" value="${f:h(dataMap[field])}"  />
           			</c:if>
					...
					<c:if test="${field == 'updated_at'}">
           				...
           				<input type="hidden" name="${f:h(field)}" value="<fmt:formatDate value="${date}" pattern="yyyy-MM-dd HH:mm:ss" />"  />
           			</c:if>
           			<c:if test="${field == 'updated_by'}">
           				<input type="text" value="${userInfo.user_id}"  class="updateinput" disabled/>
           				<input type="hidden" name="${f:h(field)}" value="${f:h(userInfo.user_id)}"   />
           			</c:if>
           			<!--   主キー,更新情報以外 -->
           			<c:if test="${column.primary_key_sign != 1 && field != 'updated_at' && field != 'updated_by'}">

						<!--   作成日の場合 -->
	       				<c:if test="${field == 'created_at'}">
							<input type="text" id = "${f:h(field)}" name="${field}" value="${f:h(dataMap[field])}" class="updateinput"/>
							<a href="javascript:openCalendar('${f:h(field)}');"><img src="${f:url('/images/calendar.jpeg')}"  align="absmiddle" width="20"  height="22" ></a>
		       			</c:if>
		       			<!--   作成者の場合 -->
		       			...
		       			<!--   作成情報以外の場合 -->
	       				<c:if test="${field != 'created_at' && field != 'created_by'}">
	       					<!--   タイプがdatetimeの場合カレンダー表示 -->
	       					<c:if test="${type == 'datetime'}">
								<input type="text" id = "${f:h(field)}" name="${f:h(field)}" value="${f:h(dataMap[field])}" class="updateinput"/>
								<a href="javascript:openCalendar('${f:h(field)}');"><img src="${f:url('/images/calendar.jpeg')}"  align="absmiddle" width="20"  height="22" ></a>
		       				</c:if>
		       				<!--   タイプがnvarcharの場合入力モードを変えるようにする -->
		       				<c:if test="${type == 'nvarchar(n)'}">
		       					<div id = "${field}">
			       					<!--  <input type="text" id="update${field}" name="${field}"   value="${f:h(dataMap[field])}"  class="updateinput"/>-->
			       					<textarea name="${field}" id = "input${field}"  class="updateinput">${f:h(dataMap[field])}</textarea>
			       					<a href="javascript:changeInput('${f:h(field)}');"><img src="${f:url('/images/textmode.jpeg')}"  align="absbottom" width="20"  height="22" ></a>
       						...
       						<c:if test="${type != 'nvarchar(n)' && type != 'datetime'}">
		       					<c:if test="${column.autoincrement_sign == 1}">
									<input type="text" id = "${f:h(field)}"  value="自動生成" name="${f:h(field)}" class="updateinput"  disabled/>
		           				</c:if>
			...
			<tr>
				<td class="btn" colspan="2"  align="center">
					<input type="submit"  value="更新確認" style="font-size=20px;"  onclick="editconfirm();" />
