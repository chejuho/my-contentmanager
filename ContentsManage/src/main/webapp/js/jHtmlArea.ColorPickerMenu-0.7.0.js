    ...
    if (jHtmlArea) {
        ...
        jHtmlArea.fn.forecolor = function(c) {
            if (c) {
                ...
                var that = this;
                var hidden = this.isTextAreaHidden();
                var pos;
                if (!hidden) {
                	pos = this.getAreaRangeText();
                }
                        ...
                        if ($.browser.msie) {
                        	...
                        	if (hidden) {
                        		...
                        		if (range || pos.start != pos.end) {
                        			insertNode = '<span style="color:' + color + '">'+ range + '</span>';
                        			textarea.val(beforeNode + insertNode + afterNode);
                        		} else if (pos.start == pos.end) {
                        			insertNode = '<span style="color:' + color + '">' + '</span>';
                        			textarea.val(beforeNode + insertNode + afterNode);
                        		}
                        	}
                        } else {
    ...
    menu.fn = menu.prototype = {
        ...
        init: function(ownerElement, options) {
            ...
            picker.appendTo(owner.parent()).
                ...
                mouseout(function() {
                    autoHide = true;
                    that.currentTimeout = window.setTimeout(function() { if (autoHide === true) { that.hide(); } }, 100);
                }).
                mouseover(function() {
